...
jx step bdd --use-revision --versions-repo https://github.com/deploy-snippet/jenkins-x-versions.git --config jx/bdd/ekstekton/cluster.yaml --base-domain=jxbdd.beescloud.com --gopath /tmp --git-provider=github --git-username $GH_USERNAME --git-owner $GH_OWNER --git-api-token $GH_ACCESS_TOKEN --no-delete-app --no-delete-repo --tests install --tests test-create-spring
